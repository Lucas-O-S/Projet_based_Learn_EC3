...
namespace ProjetilTeste
{
    ...
    public partial class Simulador : Form
    {
        ...
        public void Salvar()
        {
            Cerebro.SalvarDados(tempo,NSalvo);
            NSalvo = false;
        ...
        private void Inicializar()
        {
            ...
            Cerebro.Definir(V0,Angulo);
            ImgProj.Location = new Point(Chao.Location.X, Chao.Location.Y );
            ImgMet.Location = new Point(Cerebro.PrepMeteoro().Item1 + Chao.Location.X, -Cerebro.PrepMeteoro().Item2 + Chao.Location.Y -50 );
            Fundo.Invalidate();
        }
        ...
        public void animacao()
        {
            ...
            ImgMet.Top += Convert.ToInt32(Cerebro.MetPosY(tempo));



            //Soma valor a esquerda do projetil que empurra a direita
            ImgProj.Left += Convert.ToInt32(Cerebro.ProjPosX(tempo));

            //Subtrai o valor de cima do projetil fazendo ele subir
            ImgProj.Top -= Convert.ToInt32(Cerebro.ProjPosY(tempo));
        ...
        {
            ProjX.Text = Cerebro.getPosicaoProjX();
            ProjY.Text = Cerebro.getPosicaoProjY();

            MetX.Text = Cerebro.getPosicaoMetX();
            MetY.Text = Cerebro.getPosicaoMetY();



            TempoTotal.Text = $"Tempo Total: {tempo}";
            TempoSubida.Text = $"Tempo Subida: {Cerebro.Temposubida}s";
            TempoDescida.Text = $"Tempo Descida: {Cerebro.Tempodescida}s";
        ...
        private void Queda()
        {
            ...
            if (Cerebro.ProjCaiu() && Cerebro.MetCaiu())
            {
