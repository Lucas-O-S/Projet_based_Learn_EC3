...
namespace ProjetilTeste
{
    public static class FormulasFisica
    {
        ...
        {
            Double vx0 = V0 * Angulo.Cos(angulo, Cerebro.pres);
        ...
        {
            double SY = Angulo.Tan(angulo,Cerebro.pres) * SX - (G / (2 * Math.Pow(V0, 2) * Math.Pow(Angulo.Cos(angulo,Cerebro.pres), 2))) * Math.Pow(SX,2) ;
            return SY;
            /*
            SY = Trajetoria
            Angulo = Angulo
